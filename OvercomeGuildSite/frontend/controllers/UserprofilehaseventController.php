...
class UserprofilehaseventController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        $userProfileId = Userprofile::findOne(['user_id' => Yii::$app->user->getId()])->getId();
        $model = $this->findModel($userProfileId, $eventId);
        if ($model != null) {
            ...
            $model->role = $eventRole;
            $model->userprofile_id = Userprofile::findOne(['user_id' => Yii::$app->user->id])->getId();
            $model->attending = $eventOption;
