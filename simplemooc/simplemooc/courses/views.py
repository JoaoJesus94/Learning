...
def enrollment(request, slug):
    ...
    if created:
        # enroll.active()
        messages.success(request, 'Applied with success')
    else:
        messages.info(request, 'You\'re already in this course')
...
def undo_enrollment(request, slug):
    ...
    if request.method == 'POST':
        enroll.delete()
        messages.success(request, 'Your apply has been canceled with success')
        return redirect('accounts:dashboard')
...
def show_announcement(request, slug, pk):
    ...
    if form.is_valid():
        ...
        comment.save()
        form = CommentForm()
        messages.success(request, 'Comment crated with success')
